...
//		result.add("name", ci.getName()));
	...
	public JsonElement caseConnection(Connection c) {
		...
		result.add("bidirectional", c.isBidirectional() ? new JsonPrimitive(true) : new JsonPrimitive(false));
	...
	private JsonElement genPropertyExpression(PropertyExpression v) {
		if (v instanceof NamedValue) {
			...
			return new JsonPrimitive(Long.toString(((IntegerLiteral) v).getValue()));
...
//			return new JsonPrimitive(((ClassifierValue) v).getClassifier().getName());
	...
	public JsonElement caseProperty(Property p) {
		...
		result.add("inherit", p.isInherit() ? new JsonPrimitive(true) : new JsonPrimitive(false));
