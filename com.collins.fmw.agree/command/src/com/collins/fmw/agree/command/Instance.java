 ...
 * </copyright>
 *
 * $Id: Instance.java,v 1.190 2010-05-12 20:09:29 lwrage Exp $
 *
 */
...
import org.osate.aadl2.instantiation.CreateConnectionsSwitch;
...
public class Instance {
	 ...
	 * @param errMgr
	 */
	public Instance(final IProgressMonitor pm) {
	...
	public Instance(final IProgressMonitor pm, final AnalysisErrorReporterManager errMgr) {
		classifierCache = new HashMap<InstanceObject, InstantiatedClassifier>();
		mode2som = new HashMap<ModeInstance, List<SystemOperationMode>>();
		errManager = errMgr;
		monitor = pm;
	...
	public static SystemInstance buildInstanceModelFile(final ComponentImplementation ci, IProgressMonitor monitor,
		...
		Resource res = null;
		try {
			res = rset.getResource(instanceURI, true);
		} catch (RuntimeException e) {
			res = rset.getResource(instanceURI, false);
			if (res == null) {
				res = rset.createResource(instanceURI);
			}
		}
		...
		final Instance Instance = new Instance(monitor, new AnalysisErrorReporterManager(
				new MarkerAnalysisErrorReporter.Factory(AadlConstants.INSTANTIATION_OBJECT_MARKER)));
		return Instance.createSystemInstance(ci, res);
	}

	public static SystemInstance buildInstanceModelFile(ComponentImplementation ci, ResourceSet rset)
			throws Exception {
		return buildInstanceModelFile(ci, new NullProgressMonitor(), rset);
	}
	...
	public static SystemInstance instantiate(ComponentImplementation ci, AnalysisErrorReporterManager errorManager,
		...
		// now instantiate the rest of the model
		final Instance Instance = new Instance(monitor, errorManager);
		return Instance.createSystemInstanceInt(ci, aadlResource, false);
	}
	...
	public static SystemInstance rebuildInstanceModelFile(final IResource ires) throws Exception {
		...
		final Instance Instance = new Instance(new NullProgressMonitor(), new AnalysisErrorReporterManager(
				new MarkerAnalysisErrorReporter.Factory(AadlConstants.INSTANTIATION_OBJECT_MARKER)));
		SystemInstance root = Instance.createSystemInstance(ci, res);
	...
	public static void rebuildAllInstanceModelFiles(final IProgressMonitor monitor) throws Exception {
		...
		for (int i = 0; i < instanceRoots.size(); i++) {
			...
			Instance.createSystemInstance(ci, res);
		}
	}
	...
	public void fillSystemInstance(SystemInstance root) throws InterruptedException {
		...
		createSystemOperationModes(root, 1);
	...
	private void processConnections(SystemInstance root) throws InterruptedException {
		...
		for (ConnectionInstance conni : connilist) {
			...
			if (setPA == null && patternPA == null) {
				// OsateDebug.osateDebug("[Instance] processConnections");
...
//verbose exception message
				...
				if (patternName.equalsIgnoreCase("One_To_One")) {
					...
					for (long i = 1; i <= srcSizes.get(srcOffset); i++) {
						srcIndices.add(i);
						dstIndices.add(new Long(i == 1 ? srcSizes.get(srcOffset) : i - 1));
						result &= interpretConnectionPatterns(conni, isOpposite, patterns, offset + 1, srcSizes,
								srcOffset + 1, dstSizes, dstOffset + 1, srcIndices, dstIndices);
						dstIndices.remove(dstOffset);
						srcIndices.remove(srcOffset);
					...
					for (long i = 1; i <= srcSizes.get(srcOffset); i++) {
						srcIndices.add(i);
						dstIndices
								.add(new Long(i == 2 ? srcSizes.get(srcOffset)
...
//		OsateDebug.osateDebug ("[Instance] addUsedPropertyDefinitions=" + root);

		TreeIterator<Element> it = EcoreUtil.getAllContents(Collections.singleton(root));
		while (it.hasNext()) {
			EObject ao = it.next();
			// OsateDebug.osateDebug ("[Instance] ao=" + ao);

			if (ao instanceof PropertyAssociation) {
				// OsateDebug.osateDebug ("[Instance] ao=" + ao);

				Property pd = ((PropertyAssociation) ao).getProperty();
				// OsateDebug.osateDebug ("[Instance] pd=" + pd);
